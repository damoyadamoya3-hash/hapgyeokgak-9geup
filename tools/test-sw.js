...
async function main(){
  ...
  const current = [...good.buckets.keys()].find(k => k.startsWith('hg9-') && k !== 'hg9-v6');
  ...
  assert.strictEqual(good.buckets.has('hg9-v6'), false, '중복 셸이 든 예전 합격각 캐시가 남음');
  ...
  assert.strictEqual(broken.buckets.has('hg9-v6'), true, '설치 실패가 기존 캐시를 지움');
