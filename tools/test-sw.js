...
function makeEnv(failAssets = [], failPuts = false){
  ...
  const buckets = new Map([
    ['hg9-v3', new Map([['./index.html', new Response('old shell')]])],
...
async function main(){
  ...
  const current = [...good.buckets.keys()].find(k => k.startsWith('hg9-') && k !== 'hg9-v3');
  ...
  assert.strictEqual(good.buckets.has('hg9-v3'), false, '예전 합격각 캐시가 남음');
  ...
  assert.strictEqual(broken.buckets.has('hg9-v3'), true, '설치 실패가 기존 캐시를 지움');
